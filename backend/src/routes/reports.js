...
router.get('/mapa', async (req, res) => {
  try {
    ...
    if (lat && lng) {
      ...
      queryParams.push(lng, lat, radio * 1000); // radio en metros
      paramIndex += 3;
    }
...
router.get('/', async (req, res) => {
  try {
    ...
    let whereConditions = ['r.visible_publicamente = true'];
    let queryParams = [];
    let paramIndex = 1;
...
router.patch('/:id/estado', authenticateToken, async (req, res) => {
  try {
    ...
    const reporteResult = await query(`
      ...
      WHERE id = $1 AND visible_publicamente = true
    `, [id]);
